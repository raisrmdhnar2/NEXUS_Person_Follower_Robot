...
======================================================
File: esp32_uart.py (also mirrored in raspberry_pi/communication/esp32_uart.py)
Adheres to:
...
def main():
    """CLI testing tool for ESP32 UART bridge."""
    print("Testing Esp32UartBridge...")
    bridge = Esp32UartBridge(port="auto", enabled=True)
    print(f"Status: {bridge.status_label}")

    test_commands = ["s", "-", "x", "+", "s"]
    for c in test_commands:
        bridge.send_command(c, force=True)
        print(f"Sent command: '{c}' | Total sent: {bridge.total_bytes_sent} bytes")
        time.sleep(0.1)

    bridge.close()
    print("UART Bridge test completed successfully!")
